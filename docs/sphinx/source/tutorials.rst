...
we have prepared several integration test systems:
    ...
    * ``PointParticle_Polymer_BD_HI``: Single polymer chain diffusing in a slit channel with HI considered.

    * ``PointParticle_Stokes_GGEM``: GGEM validation of Stokes system

    * ``RigidParticle_Sphere_Sedimentation_HI``: Single spherical particle sedimenting in a slit channel with HI considered.

    * ``RigidParticle_Sphere_Pressure_Flow``: single spherical particle moving in a channel with pressure driven flow.

    * ``PointParticle_Poisson_GGEM``: GGEM validation of Poisson System
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    2. Create corresponding ``input files``, ``run.sh`` and ``zclean.sh`` under the
     new folder

    3. Run the simulation use the new files created in step 2 and store
    necessary outputs under folder, ``$COPSS_DIR/tests/integration_tests/resources/NEW_TEST/output``

    4. Modify ``$COPSS_DIR/tests/integration_tests/json/test.json`` to include
    the new test. The new dictionaries added to this json file has to comply
    with the schema file ``$COPSS_DIR/tests/integration_tests/json/test_schema.json``, otherwise the test will not pass the validation.

    5. One can also turn on or off a test by changing the ``on`` field in
    ``$COPSS_DIR/tests/integration_tests/json/test.json`` file to ``true`` or
    ``false``. The integration test will only test systems that are turned on.

    6. Testing the new integration test.
