...
{
  particle_type =
    pm_system->get_equation_systems().parameters.get<std::string>("particle_type");
...
void FixPoint::initParticleType()
{
  if (particle_type != "point_particle") {
    ...
    " is only for point_particle, i.e., cannot be applied to particle type = " <<
  ...
  if (wall_type == "slit") {
      ...
      for (std::size_t i = 0; i < dim; ++i)
      {
        ...
        else // non-periodic boundary (inpenetrable wall)
        {
          if (pt0(i) <  box_min(i)) {
            ...
            std::cout << "*** Warning: " << p_id <<
            "-th bead is out of domain. Out of domain occurs "
    ...
    for (std::size_t id = 0; id < num_points; ++id) {
      ...
      if (pt0_norm > cavity_radius) {
        ...
        out_domain_counter += 1;
        std::cout << "*** Warning: " << p_id << "-th bead (" << pt0(0) << "," <<
        pt0(1) << "," << pt0(2) << ") is out of domain. Out of domain occurs "
                  << out_domain_counter << " times in total." << std::endl;
      }
    } // end for loop
