...
FixPointDiscretizedWallLJCut::FixPointDiscretizedWallLJCut(
...
{
  START_LOG("FixPointDiscretizedWallLJCut::initParams()",
            "FixPointDiscretizedWallLJCut");
  force_params =
    pm_system->get_equation_systems().parameters.get<std::vector<Real> >(
      "discretized_wall/lj_cut");

  if (force_params.size() != 4) {
    std::cout << std::endl <<
      "********************Error message********************" << std::endl
              <<
      "---------------> The force type 'discretized_wall/lj_cut' requires 4 parameter (epsilon, sigma, rcut, num_wall_particles) (dimensionless form)"
              << std::endl
              << "****************************************" << std::endl;
    libmesh_error();
  }
  ...
  STOP_LOG("FixPointDiscretizedWallLJCut::initParams()",
           "FixPointDiscretizedWallLJCut");
}

void FixPointDiscretizedWallLJCut::print_fix()
{
  START_LOG("FixPointDiscretizedWallLJCut::print_fix()",
            "FixPointDiscretizedWallLJCut");
  std::cout << "this is FixPointDiscretizedWallLJCut" << std::endl;
  STOP_LOG("FixPointDiscretizedWallLJCut::print_fix()",
           "FixPointDiscretizedWallLJCut");
}

void FixPointDiscretizedWallLJCut::compute()
{
  START_LOG("FixPointDiscretizedWallLJCut::compute()",
            "FixPointDiscretizedWallLJCut");
  ...
  for (std::size_t p_id = 0; p_id < num_points; ++p_id)
  {
    ...
    {
      const Point r_ij = point_mesh->pm_periodic_boundary()->point_vector(pti,
                                                                          wall_particle_pos[
                                                                            j]);
...
{
  START_LOG("FixPointDiscretizedWallLJCut::read_wall_particle()",
            "FixPointDiscretizedWallLJCut");
  ...
  {
    printf(
      "***warning: FixPointDiscretizedWallLJCut::read_wall_particle() can NOT read the wall_particle data!");
    libmesh_error();
  }

  // Read beads data
  for (unsigned int i = 0; i < num_wall_particles; i++) {
    infile >> wall_particle_pos[i](0) >> wall_particle_pos[i](1) >>
    wall_particle_pos[i](2);
  }

  // Finish and close the file
  infile.close();
  std::cout << "Reading wall particle positions from " <<
    wall_particle_filename << " is completed!\n\n";


  STOP_LOG("FixPointDiscretizedWallLJCut::read_wall_particle()",
           "FixPointDiscretizedWallLJCut");
}
