...
FixPointSlitWallEmpiricalDNA::FixPointSlitWallEmpiricalDNA(
...
{
  START_LOG("FixPointSlitWallEmpiricalDNA::initPointParticleType()",
  ...
  STOP_LOG("FixPointSlitWallEmpiricalDNA::initPointParticleType()",
           "FixPointSlitWallEmpiricalDNA");
}

void FixPointSlitWallEmpiricalDNA::initParams()
{
  START_LOG("FixPointSlitWallEmpiricalDNA::initParams()",
            "FixPointSlitWallEmpiricalDNA");
  wall_params =
  ...
  for (int i = 0; i < dim; i++) {
    if (box_min(i) != wall_params[2 * i] or box_max(i) !=
      ...
      "********************Error message********************" << std::endl
                <<
      "for force field 'wall/empirical_dna' (slit), wall_params has to be the same as box_min and box_max"
                << std::endl
                << "****************************************" << std::endl;
      libmesh_error();
    }
  ...
  d0 = 0.5 / c2;
  c0 = 25.0 * c1;
  STOP_LOG("FixPointSlitWallEmpiricalDNA::initParams()",
           "FixPointSlitWallEmpiricalDNA");
}

void FixPointSlitWallEmpiricalDNA::print_fix()
{
  START_LOG("FixPointSlitWallEmpiricalDNA::print_fix()",
            "FixPointSlitWallEmpiricalDNA");
  std::cout << "this is FixPointSlitWallEmpiricalDNA" << std::endl;
  STOP_LOG("FixPointSlitWallEmpiricalDNA::print_fix()",
           "FixPointSlitWallEmpiricalDNA");
}

void FixPointSlitWallEmpiricalDNA::compute()
{
  START_LOG("FixPointSlitWallEmpiricalDNA::compute()",
            "FixPointSlitWallEmpiricalDNA");
  ...
  STOP_LOG("FixPointSlitWallEmpiricalDNA::compute()",
           "FixPointSlitWallEmpiricalDNA");
}
