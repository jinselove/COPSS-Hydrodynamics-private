  ...
  for (std::size_t i = 0; i < num_points; ++i)
  {
    ...
    for (std::size_t j = 0; j < dim; ++j) {
      ...
      if (periodic[j] == false and inlet[j] == false) {
        r_i_lo(j) = box_min(j) - pti(j); // opposite sign compared to rij = rj -
                                         // r_i
        r_i_hi(j) = box_max(j) - pti(j); // opposite sign compared to rij = rj -
                                         // r_i

        if (r_i_lo.norm() <= rCut) pforce(j) += fix_base.lj_force(r_i_lo,
                                                                  epsilon,
                                                                  sigma)(j);

        if (r_i_hi.norm() <= rCut) pforce(j) += fix_base.lj_force(r_i_hi,
                                                                  epsilon,
                                                                  sigma)(j);
      } // end of
    }   // end for (i<_dim)
