...
void FixPointSphereWallLJCut::initParams()
{
  ...
  if (wall_params.size() != 1) {
    std::cout << std::endl <<
      "********************Error message********************" << std::endl
              <<
      "---------------> The force type 'wall/lj_cut' (sphere) requires 1 wall parameter, i.e., radius (dimensionless form)"
              << std::endl
              << "****************************************" << std::endl;
    libmesh_error();
  }
...
void FixPointSphereWallLJCut::compute()
{
  ...
  for (std::size_t i = 0; i < num_points; ++i)
  {
    ...
    if (r_i_sphereWall.norm() <= rCut) pforce = fix_base.lj_force(r_i_sphereWall,
                                                                  epsilon,
                                                                  sigma);
