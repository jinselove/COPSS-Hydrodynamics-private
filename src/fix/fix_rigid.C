  ...
  if (particle_type != "rigid_particle") {
    std::cout << std::endl <<
      "*******************Error message*********************" << std::endl
              << "The force field : " << force_type <<
      " is only for rigid_particle, i.e.,"
...
void FixRigid::check_walls() {
  ...
  if (wall_type == "slit") {
    ...
    {
      /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
         1) Check if this particle is on the periodic boundary. If so, move the
            nodes
         - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            -*/
      ...
      if (on_pb)
      {
        ...
        std::cout << " >>>>> Notice: particle " << i <<
          "is on the period boundary, particle mesh is rebuilt" << std::endl;
      }
      ...
      for (; nd != end_nd; ++nd) {
        ...
        for (std::size_t i = 0; i < dim; i++)
        {
          if (!periodic[i]) {
            if ((*node)(i) <
                box_min(i)) (*node)(i) = 2. * box_min(i) - (*node)(i);

            if ((*node)(i) >=
                box_max(i)) (*node)(i) = 2. * box_max(i) - (*node)(i);
          }
        }
    ...
    for (std::size_t i = 0; i < num_rigid_particles; ++i)
    {
      ...
      for (; nd != end_nd; ++nd) {
        ...
        if (node->norm() >
            cavity_radius) (*node) = node->unit() * (2. * cavity_radius) -
                                     (*node);
      } // end for loop over nodes
    }   // end loop over particles
  }     // end else
...
// fix this: when rigid particle crosses non-slip walls, we need to find the
// farest node
...
//       /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - -
//         1) Check if this particle is on the periodic boundary. If so, move
// the nodes
//        - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// -*/
//       const bool on_pb = rigid_particles[i]->on_the_periodic_boundary();
...
//       {
//         rigid_particles[i]->rebuild_periodic_mesh();
//         std::cout<<" >>>>> Notice: particle " <<i << "is on the period
// boundary, particle mesh is rebuilt" <<std::endl;
//       }
//       /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// - -
//         2) Check if this particle penetrates non-slip wall, If so, move
// center of mass and all nodes
//        - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// -*/
...
void FixRigid::sync_node_to_pointmesh()
{
  ...
  for (std::size_t i = 0; i < num_rigid_particles; ++i)
  {
    ...
    const std::size_t n_nodes =
      particle_mesh->particles()[i]->num_mesh_nodes();
    ...
    for (; nd != end_nd; ++nd)
    {
      ...
      point_mesh->particles()[point_start_id +
                              node_id]->add_particle_force(gforce);

      //  ------------------ TEST: print out the nodal force
      // ----------------------
      // if(pm_system->comm().rank()==0){
      //   printf("--->TEST:reinit_force_field() gforce =
      ...
      // test_centroid_force(test_dim) +=
      // point_mesh->particles()[test_i]->particle_force()[test_dim];
      // }
      // printf("Debug: for particle %d, test_centroid_force = (%f, %f,
      // %f)\n",i, test_centroid_force(0), test_centroid_force(1),
      // test_centroid_force(2) );
