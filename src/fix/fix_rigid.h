...
public:
  ...
  /*
   * 1) check if a rigid particle is on a periodic boundary, if so, rebuild the
   *particlemesh (fix me, is this necessary)
   * 2) check if a particle is on or crosses a non-slip wall, if so, move the
   *nodes that crosses the wall back
   */
  void check_walls();
