    ...
    for (; nd != end_nd; ++nd){
      ...
      for(std::size_t j=0; j<dim; ++j){
        ...
        if(periodic[j]==false and inlet[j]==false){
          ...
          if(r_i_lo.norm() <= rCut) pforce[node_id](j) += fix_base.lj_force(r_i_lo, epsilon,sigma)(j);
          if(r_i_hi.norm() <= rCut) pforce[node_id](j) += fix_base.lj_force(r_i_hi, epsilon,sigma)(j);
        }
      }
    } // loop over all nodes
