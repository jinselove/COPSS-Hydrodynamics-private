...
FixRigidSurfaceConstraint::FixRigidSurfaceConstraint(
...
{
  START_LOG("FixRigidSurfaceConstraint::initParams()",
  ...
  if (force_params.size() != 2) {
    ...
    "*******************Error message*********************" << std::endl
              <<
    "'surface_constraint' needs two parameters: surface_spring_constant, center_spring_constant"
              << std::endl
              << "****************************************" << std::endl;
    libmesh_error();
  }
  ...
  STOP_LOG("FixRigidSurfaceConstraint::initParams()",
           "FixRigidSurfaceConstraint");
}

void FixRigidSurfaceConstraint::print_fix()
{
  START_LOG("FixRigidSurfaceConstraint::print_fix()",
            "FixRigidSurfaceConstraint");
...
{
  START_LOG("FixRigidSurfaceConstraint::compute_constraint_force()",
            "FixRigidSurfaceConstraint");
  ...
  for (std::size_t j = 0; j < n_nodes; ++j)
  {
    ...
    for (std::size_t k = 0; k < node_neighbors.size(); ++k)
    {
      ...
      const Point  R_ij           =
        particle_mesh->pm_periodic_boundary()->point_vector(pt0, ptk);
      Point sf = fix_base.spring_force_lhs(R_ij,
                                           neigh_l0,
                                           surface_spring_constant);
      spring_f += sf;
    } // end for k-loop
    ...
    const Point Rc_ij = particle_mesh->pm_periodic_boundary()->point_vector(pt0,
                                                                            p_centroid);
    Point sfc = fix_base.spring_force_lhs(Rc_ij,
                                          lc0,
                                          center_spring_constant);
    ...
    // {
    //   printf("--->test in ForceField::rigid_constraint_force(), Particle
    // %lu\n",i);
    //   printf("--->Spring force (surface+center=total) on node %lu is fs =
    // (%f,%f,%f)\n",
    //          j,spring_f(0),spring_f(1),spring_f(2);
    //   printf("--->Spring force (center) on node %lu is               fs =
    // (%f,%f,%f)\n",
    //          j,sfc(0),sfc(1),sfc(2);
    // }
  } // end for j-loop
  STOP_LOG("FixRigidSurfaceConstraint::compute_constraint_force()",
           "FixRigidSurfaceConstraint");
}
