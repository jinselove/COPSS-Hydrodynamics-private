...
void GGEMPoisson::set_ksi()
{
    ...
    {
        std::cout << "Error: error in GGEMPoisson::set_ksi(), required parameters\
    ...
    switch (point_type)
    {
      ...
      {
        std::cout << "Error: ksi is not defined for LAGRANGIAN_POINT yet" <<std::endl;
        libmesh_error();
      }
      default:
      {
        std::cout << "Error: error in GGEMPoisson::set_ksi(): Undefined PointType!" << std::endl;
        libmesh_error();
      }
    }
                                                ...
                                                ) const
{
    ...
    if(r < r_eps) {
        std::cout << "Warning: warning in GGEMPoisson::green_tensor_exact, \
                                      ...
                                      const std::string& charge_type) const
{
    ...
    for (std::size_t v=0; v<IndicesDists.size(); ++v)
    {
        ...
        if(charge_type=="regularized")
          ...
          libmesh_assert("GGEMPoisson::local_potential_bead, wrong charge_type");
