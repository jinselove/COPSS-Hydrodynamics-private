...
public:
   ...
   * In dimensionless form, G = 1 / r, the later r is the first r
   *non-dimensionalized
  ...
  /*
   * Green tensor for unbounded domain due to a smoothed charge defined by a
   *Gaussian
   ...
   * Reference: The ksi part of Equation (17) in Membrane paper, Jarol E Molina,
   *J de Pablo, J.Hernandez-Ortiz
   * The reason why we only take the ksi part is because Equation (17) is the
   *local part of the green
   * tensor, i.e, the green tensor because of the local charge, i.e., g_ski -
   ...
   * For ksi^-1 = a/3. The charge is distributed through a gaussian function
   *with a
   * valence relative to the particle/bead diameter.
   * Reference: Equation (17) in Membrane paper, Jarol E Molina, J de Pablo,
   *J.Hernandez-Ortiz
   * The dimensionless unit of G is 1 / (4*pi*epsilon*epsilon_0)
   */
  ...
  */
  ...
  // ! set phi0
  void set_phi0(const Real& _phi0) {
    phi0 = _phi0;
  }
...
private:
  ...
  // ! potential field dimensionless unit, i.e., the value of e /
  // (4*pi*epsilon*epsilon_0)
  Real phi0 = NAN;
};
} // end of namespace
