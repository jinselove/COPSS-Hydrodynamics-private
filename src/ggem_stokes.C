...
void GGEMStokes::set_ksi()
{
    ...
    {
        std::cout << "Error: error in GGEMStokes::set_ksi(), required parameters\
    ...
    switch (point_type)
    {
      ...
      {
        ksi = 1.0 / (ibm_beta * hmin);  // default value of ibm_beta is 0.75, which needs to be optimized for different number of nodes
        break;
      }
      default:
      {
        std::cout << "Error: error in GGEMStokes::set_ksi(): Undefined PointType!" << std::endl;
        libmesh_error();
      }
    }
                                                                ...
                                                                ) const
{
    ...
    if(r < r_eps) {
        std::cout << "Warning: warning in GGEMStokes::green_tensor_exact, \
    ...
    for (int i=0; i<dim; ++i)
    {
        for (int j=0; j<dim; ++j){
          ...
          G(i,j) = one_eight_pi_mu / r *( _kronecker_delta[i][j] + x(i)*x(j)/r2 ); // equation (2) in Juan. Hernandez-Ortiz, PRL 98.140602 (2007)
        }
    }
                                                   ...
                                                   const bool& zero_limit) const
{
    ...
    if (zero_limit)
    {
        for (int i=0; i<dim; ++i)
            for (int j=0; j<dim; ++j)
                G(i,j) = coeff_1 * _kronecker_delta[i][j];
    } // end if()
    else
    {
        // coefficients
                                                               ...
                                                               const bool& zero_limit) const
{
    ...
    else
    {
        ...
        const Real c2 = two_ksi_sqrt_pi * std::exp(-kr2) - two_alpha_sqrt_pi * std::exp(-ar2);
        // compute G tensor; since G is symmetric, we will built the upper half and
        // the diagonal element first, and do a mirror
        Real tmp1 = 0, tmp2 = 0, xixj_r2 = 0.;
        for (int i=0; i<dim; ++i)
...
//   GTk = this->green_tensor(x, reg_param,   mu, dim, zero_limit, delta_type_k);
//   GTk -= GTa;
// 
