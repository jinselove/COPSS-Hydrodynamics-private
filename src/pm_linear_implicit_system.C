...
// ==================================================================================
void PMLinearImplicitSystem::write_out_single_particle(
  const Point            & coords,
...
const
{
  ...
  // the following steps, ios_base::app, output operations happen at the end of
  // the file,
...
const
{
  ...
  //  PetscPrintf(PETSC_COMM_WORLD,"--->test in write_out_particle_coordinate
  // vin = \n");
...
const
{
  ...
  if (this->comm().rank() == 0)
  {
    ...
    if (write_velocity) outfile << ",  Vx,      " << "Vy,       " <<
        "Vz,      " << "Vmag ";
    outfile << "\n";
...
// ==================================================================================
PetscErrorCode PMLinearImplicitSystem::write_point_csv(
  const std::string& filename,
  Vec               *petsc_vec,
  const bool         write_velocity)
const
