...
public:
  ...
  void set_particle_center(const Point& center) {
    _center = center;
  }
  ...
  void set_pbc_counter(int nx, int ny, int nz) {
    _counter[0] = nx; _counter[1] = ny; _counter[2] = nz;
  }
   ...
   * This can be either a point on immersed bodies or a bead on the polymer
   *chains.
   *
   * Note that points with the same type may have different parent id. For
   *example,
   * They can be bead type points on two different polymer chains, or tracking
   *points
   ...
   * (1)_center; (2) force; (3)_counter; (4)_id; (5)_parent_id; (6)_point_type;
   *(7)_orientation;
   */
...
private:
  ...
  // the force vector excerted on this particle(non-hydrodynamic and
  // non-Brownian)
  Point _force;

  // constant force vector excerted on the particle (non-hydrodynamic and
  // non-Brownian)
  // If 'p_constant' is added to the force_field, this force will be applied to
  // the particle
  // at each timestep
  ...
  // Knowing the parent's property will facilitate the computation of force
  // field.
